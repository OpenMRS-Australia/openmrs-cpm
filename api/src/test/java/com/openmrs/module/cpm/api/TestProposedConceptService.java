...
public class TestProposedConceptService extends CpmBaseContextSensitive {
    ...
    protected Comment getMockComment(Integer id, String text, CommenterRole commenterRole) {
    ...
    @Test
    public void saveProposedConceptPackage_saveWithChildConceptAndComments() throws Exception {
        ProposedConceptPackage testPackage = getMockProposedConceptPackage(null, "new package");
        List<Concept> concepts = conceptService.getAllConcepts();
        log.debug("************************************************************");
        for (Concept currentConcept : concepts) {
        ...
        String conceptName0 = "concept 0";
        ProposedConcept concept1 = getMockProposedConcept(null, conceptName0, "concept 0 description", conceptService.getConcept(3));
        testPackage.addProposedConcept(concept1);

        String conceptName1 = "concept 1";
        ProposedConcept concept2 = getMockProposedConcept(null, conceptName1, "concept 1 description", conceptService.getConcept(4));
        String commentText1 = "comment1";
        Comment comment1 = getMockComment(null, commentText1, CommenterRole.PROPOSER);
        concept2.addComment(comment1);
        testPackage.addProposedConcept(concept2);

        String conceptName2 = "concept 2";
        ProposedConcept concept3 = getMockProposedConcept(null, conceptName2, "concept 2 description", conceptService.getConcept(5));
        String commentText2 = "comment2";
        Comment comment2 = getMockComment(null, commentText2, CommenterRole.PROPOSER);
        concept3.addComment(comment2);
        String commentText3 = "comment3";
        Comment comment3 = getMockComment(null, commentText3, CommenterRole.REVIEWER);
        concept3.addComment(comment3);
        testPackage.addProposedConcept(concept3);

        Map<String, Integer> conceptNames2commentCounts = new HashMap<String, Integer>();
        conceptNames2commentCounts.put(conceptName0, 0);
        conceptNames2commentCounts.put(conceptName1, 1);
        conceptNames2commentCounts.put(conceptName2, 2);

        Map<String,CommenterRole> commentTexts2CommenterRoles = new HashMap<String, CommenterRole>();
        commentTexts2CommenterRoles.put(commentText1, CommenterRole.PROPOSER);
        commentTexts2CommenterRoles.put(commentText2, CommenterRole.PROPOSER);
        commentTexts2CommenterRoles.put(commentText3, CommenterRole.REVIEWER);
        ...
        Assert.assertEquals(3, testPackage.getProposedConcepts().size());
        for ( ProposedConcept proposedConcept : testPackage.getProposedConcepts()) {
            Assert.assertNotNull(proposedConcept.getId());
            Assert.assertEquals(conceptNames2commentCounts.get(proposedConcept.getName()).intValue(), proposedConcept.getComments().size());
            for (Comment comment : proposedConcept.getComments()) {
                Assert.assertNotNull(comment.getId());
                Assert.assertEquals(commentTexts2CommenterRoles.get(comment.getText()), comment.getCommenterRole());
            }
        }
    }
    ...
    public void getProposedConceptPackageResponseById_basicRetrieval() throws Exception {
        ...
        Assert.assertEquals("Description for concept proposal package 1", testPackage.getDescription());
        Assert.assertEquals(1, testPackage.getVersion()
                                          .intValue());
        Assert.assertEquals(PackageStatus.RECEIVED, testPackage.getStatus());
        Assert.assertEquals(3, testPackage.getProposedConcepts()
                                          .size());
        Assert.assertNull(testPackage.getCreator()
                                     .getName());
    }
    ...
    public void saveProposedConceptPackageResponse_basicSave() throws Exception {
        ...
        service.saveProposedConceptResponsePackage(testPackage);
        log.info("After: " + testPackage);
        Assert.assertTrue(testPackage.getId()
                                     .intValue() >= 3);
    }
    ...
    public void saveProposedConceptPackageResponse_updateAddChild() throws Exception {
        ...
        log.info("Retrieved: " + testPackage);
        Assert.assertEquals(newName, testPackage.getName());
        Assert.assertEquals(4, testPackage.getProposedConcepts()
                                          .size());
    }
    ...
    public void saveProposedConceptPackageResponse_updateRemoveChild() throws Exception {
        ...
        String newName = "New Name";
        ProposedConceptResponse testResponse = (ProposedConceptResponse) testPackage.getProposedConcepts()
                                                                                    .iterator()
                                                                                    .next();
        ...
        log.info("Retrieved: " + testPackage);
        Assert.assertEquals(newName, testPackage.getName());
        Assert.assertEquals(2, testPackage.getProposedConcepts()
                                          .size());
    }
    ...
    @Test
    public void shouldReturnEmptyListWhenNoStatusRequested() {
        // Given

        // When
        final List<ProposedConceptPackage> packages = service.getProposedConceptPackagesForStatuses();
    ...
    @Test
    public void shouldReturnAllReceivedProposalPackages() throws Exception {
        // Given

        // When
        final List<ProposedConceptPackage> packages = service.getProposedConceptPackagesForStatuses(PackageStatus.RECEIVED);
    ...
    @Test
    public void shouldReturnAllDraftProposedPackages() {
        // Given

        // When
        final List<ProposedConceptPackage> packages = service.getProposedConceptPackagesForStatuses(PackageStatus.DRAFT);
    ...
    @Test
    public void shouldReturnAllDraftAndReceivedProposedPackages() {
        // Given

        // When
        final List<ProposedConceptPackage> packages = service.getProposedConceptPackagesForStatuses(PackageStatus.DRAFT, PackageStatus.RECEIVED);
